...
void LcdClrRect(INT16 usLeft, INT16 usTop, INT16 usRight, INT16 usBottom, INT8U ucColor)
{
    ...
    INT32U ulColor = (ucColor << 28) | (ucColor << 24) | (ucColor << 20) | (ucColor << 16) | (ucColor << 12) |
                     (ucColor << 8) | (ucColor << 4) | ucColor;
...
void Lcd_Draw_Line(INT16 usX0, INT16 usY0, INT16 usX1, INT16 usY1, INT8U ucColor, INT16U usWidth)
{
    ...
    if(usDx >= usDy)
	{
	    for(wCurx = usX0, wCury = usY0, wNextx = usX1, wNexty = usY1, decision = (usDx >> 1); wCurx <= wNextx;
	        wCurx++, wNextx--, decision += usDy)
		{
		    if(decision >= usDx)
			{
			    decision -= usDx;
		    ...
		    for(wpy = wNexty - usWidth / 2; wpy <= wNexty + usWidth / 2; wpy++)
			{
	...
	{
	    for(wCurx = usX0, wCury = usY0, wNextx = usX1, wNexty = usY1, decision = (usDy >> 1); wCury <= wNexty;
	        wCury++, wNexty--, decision += usDx)
		{
		    if(decision >= usDy)
			{
			    decision -= usDy;
...
void Lcd_Dma_Trans(void)
{
    ...
    //#define LCD_ACTIVE_BUFFER	LCD_VIRTUAL_BUFFER								//DMA
    //OFF #define LCD_BUF_SIZE		(SCR_XSIZE*SCR_YSIZE/2)  So the Lcd Buffer Low area is from LCD_VIRTUAL_BUFFER to
...
/*********************************************************************************************
 * name:		mostrarInstrucciones()
 * func:		Muestra las instrucciones antes de la partida
 * para:		none
 * ret:		none
 * modify:
 * comment:
...
void mostrarInstrucciones()
{
    ...
    // Caja titulo
    Lcd_Draw_Box(20, 20, 300, 67, 15);

    Lcd_DspAscII8x16(132, 25, BLACK, "REVERSI");
    Lcd_DspAscII8x16(108, 46, BLACK, "INSTRUCCIONES");

    Lcd_DspAscII8x16(20, 72, BLACK, "Boton izquierdo: Mover en vertical");
    Lcd_DspAscII8x16(20, 93, BLACK, "Boton derecho: Mover en horizontal");
    Lcd_DspAscII8x16(20, 114, BLACK, "Tocar tablero: Confirmar ficha");
    Lcd_DspAscII8x16(20, 135, BLACK, "Tocar fuera: Rendirse");

    Lcd_DspAscII8x16(88, 204, BLACK, "PULSE PARA JUGAR!!");
...
void mostrarTablero(char tablero[8][8])
{
    ...
    for(i = 0; i <= 7; i++)
	{
	    for(j = 0; j <= 7; j++)
		{
		    if(tablero[i][j] == 0)
			{
			    LcdClrRect(22 + (25 * j), 22 + (25 * i), 42 + (25 * j), 42 + (25 * i), 0);
			}
		    else if(tablero[i][j] == 1)
			{
			    BitmapView(22 + (25 * j), 22 + (25 * i), Stru_Bitmap_fBlanca);
			}
		    else
			{
			    BitmapView(22 + (25 * j), 22 + (25 * i), Stru_Bitmap_fNegra);
			}
		}
	}
    Lcd_Dma_Trans();
